...
package Bundler::Dependencies::APPLES_Datatypes;
use MooseX::Types
	-declare => [qw(
		Boolean 
		GenomicRegionType
...
subtype APPLESSpeciesName,
	as Str,
	where { ( $_ eq 'arabidopsis' ) || ( $_ eq 'lyrata' ) || ( $_ eq 'poplar' ) || ( $_ eq 'grape' ) || ( $_ eq 'rice' ) || ( $_ eq 'maize' ) || ( $_ eq 'sorghum' ) || ( $_ eq 'rice' ) || ( $_ eq 'indica' ) ||  ( $_ eq 'glaberrima' ) || ( $_ eq 'brassica' ) || ( $_ eq 'basidio' ) || ( $_ eq 'medicago' )  || ( $_ eq 'soybean' ) || ( $_ eq 'papaya' ) || ( $_ eq 'tomato' ) || ( $_ eq 'cotton' ) || ( $_ eq 'castor' ) || (exists $Hensembl_species{$_}) || ( $_ eq 'vitis_vinifera' ) || ( $_ eq 'swamp she-oak' ) },
...
subtype EvolutionaryTreeKingdom,
        as Str,
        where { ( $_ eq 'plants' ) || ( $_ eq 'fungi' ) || ( $_ eq 'vertebrates' ) || ( $_ eq 'all' ) },
