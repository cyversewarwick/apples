...
=head2 BiFa Server Interface class

BiFa Server interface for scoring Motifs.

=cut

class Motifs::BiFa_Server_Interface {
	use Runtime;
	...
	method PssmFreqs($pssmName,$dataVersion) {
		if ( $dataVersion ne $self->tfVersion ) { return 0; }
		my $var1 =
		  SOAP::Data->type('string')->name('pssmName')->value($pssmName);
...
# to the wsbc server without encryption).
		my $server = get_config_key('BiFa')->{hostname} || 'wsbc.warwick.ac.uk';
		my $port   = get_config_key('BiFa')->{port}     || 4338;
		my $username = get_config_key('BiFa')->{username};
		my $password = get_config_key('BiFa')->{password};
		...
		$self->sIf( SOAP::Lite->uri( 'http://' . $Uri )
			  ->proxy( 'http://' . $username . ':' . $password . '@' . $Uri ) );

		#	Now get the cookie, using the 'returningString' method
		my $var =
		  SOAP::Data->type('string')->name('key')->value('connection_info');
...
#	it for subsequent connections
 ...
 #	And change the soap interface data to use the cookie rather than the password
	...
	method IsFault($result) {
		if ( $result->fault ) {
			$self->errorStr( join ',', $result->faultcode,
				$result->faultdetail );
			return 1;
		}
		return 0;
...
#	to create the correct syntax
	...
	method MaxChainMaxNumSequences() {
		my $result =
		  $self->sIf->maxChainMaxNumSequences( SOAP::Data->type('none') );
		if ( $self->IsFault($result) ) { return 0; }
		return $result->result();
	}
...
# The need for the @{} syntax took hours to sort out, it extracts the underlying array
	...
	method Pssms($useConsensusSequences, $matrixSpecies, $matrixNameMatch, $dataVersion,$pssmSets) {
		if ( $dataVersion ne $self->tfVersion ) { return 0; }
		my $var1 =
		  SOAP::Data->type('boolean')->name('useConsensusSequences')
		  ->value($useConsensusSequences);
		my $var2 =
		  SOAP::Data->type('string')->name('matrixSpecies')
		  ->value($matrixSpecies);
		my $var3 =
		  SOAP::Data->type('string')->name('matrixNameMatch')
		  ->value($matrixNameMatch);
		...
		if ( defined $pssmSets ) {
			...
			if ( $i > 0 ) {
				my $var4 =
				  SOAP::Data->type('tns:stringArray')->name('pssmSets')
				  ->value($pssmSets);
...
#	location in a sequence
	method scorePssmOnSequence ($pssm_name,$sequence) {
		my $var1 =
		  SOAP::Data->type('string')->name('pssm_name')->value($pssm_name);
		my $var2 =
		  SOAP::Data->type('string')->name('sequence')->value($sequence);
...
#	only algorithm currently supported in this method.
	method scorePssmsOnSequences ($pssmNames,$sequences,$algorithm,$dataVersion) {
		...
		my $var1 =
		  SOAP::Data->type('tns:stringArray')->name('pssmNames')
		  ->value($pssmNames);
		my $var2 =
		  SOAP::Data->type('tns:stringArray')->name('sequences')
		  ->value($sequences);
		my $var3 =
		  SOAP::Data->type('integer')->name('algorithm')->value($algorithm);
	...
	method bifa ($sequence, $threshold, $title, $algorithm, $showLabels, $phyloSequences,$useConsensusSequences, $matrixSpecies,$phyloThreshold, $matrixNameMatch, $useOldAlgorithm, $useCumulativeLikelihoods, $pssmSets) {
		...
		my $var1 =
		  SOAP::Data->type('string')->name('sequence')->value($sequence);
		my $var2 =
		  SOAP::Data->type('float')->name('threshold')->value($threshold);
		my $var3 = SOAP::Data->type('string')->name('title')->value($title);
		my $var4 =
		  SOAP::Data->type('int')->name('algorithm')->value($algorithm);
		my $var5 =
		  SOAP::Data->type('boolean')->name('showLabels')->value($showLabels);
		my $var7 =
		  SOAP::Data->type('boolean')->name('useConsensusSequences')
		  ->value($useConsensusSequences);
		my $var8 =
		  ...
		  ->value($matrixSpecies);
		my $var9 =
		  SOAP::Data->type('float')->name('phyloThreshold')
		  ->value($phyloThreshold);
		my $var10 =
		  SOAP::Data->type('string')->name('matrixNameMatch')
		  ->value($matrixNameMatch);
		my $var11 =
		  SOAP::Data->type('boolean')->name('useOldAlgorithm')
		  ->value($useOldAlgorithm);
		my $var12 =
		  SOAP::Data->type('boolean')->name('useCumulativeLikelihoods')
		  ->value($useCumulativeLikelihoods);

		#	And put them into the array holding the soap data to be output
		my $var = SOAP::Data->type('complex')->value(
...
#  So, the following line sets i to the length of the array phylosequences.   But that was obvious (not)
		...
		{
			my $var6 =
			  SOAP::Data->type('tns:stringArray')->name('phyloSequences')
			  ->value($phyloSequences);
	...
	method bifaHits ($sequence, $threshold, $algorithm, $phyloSequences,$useConsensusSequences, $matrixSpecies,$phyloThreshold, $matrixNameMatch, $useCumulativeLikelihoods,$dataVersion,$pssmSets) {
		...
		#	And put them into the array holding the soap data to be output
		my $var =
		  SOAP::Data->type('complex')
		  ->value( $var1, $var2, $var3, $var5, $var6, $var7, $var8, $var9 );
